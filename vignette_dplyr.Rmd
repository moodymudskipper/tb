...
## comparing to *tidyverse* packages

The *tidyverse* has clear functions which aim mostly at doing a single thing well,
so listing its functions is like listing features and is a great test for this
package. We will go through examples from the doc and see how we can crack them.

tidyverse functions can be used on *tb* objects, the output might be a tibble
though, so both *tidyverse* and *tb*  versions of the following examples can be 
copy/pasted and if we define first:
...
|   dplyr                                 |            tb                |
|-----------------------------------------|------------------------------|
| `filter_all(mtcars, all_vars(. > 150))` | `mtcars[rowAll(.x > 150),]`  |
| `filter_all(mtcars, any_vars(. > 150))` | `mtcars[rowAny(.x > 150),]`  |

`rowAll()` and `rowAny()` are conveniences functions provided with `tb`
...
> Need to fix it so `.x` is a tb!
> Need to make `?` return logical as it now returns character
...
to sort charater value descending, use `order(-xtfrm(col))`, `xtfrm()` is a base function, or
use `dplyr::desc()` which is exactly the same.

> `mtcars[order(-wt) ~ cyl,]` doesn't make much sense but if we do it it doesn't
> return the expected result! I think I'd expect cyl not changing but all the rest reordered
...
| `select(iris, obs = starts_with('S'))`| `iris[s({?"^S"} := paste0("obs", seq_along(.)))]` | Once transmute is coded!
...
but I'll still try to explain it while we're here : 
...
* The dot place holder on the rhs will be substituted by the names of the columns
qualified by the lhs expression
...
Here again regex saves the day.

<!-- > `?` should return logical, so if needed we can do `!?` and it makes sense, but not `-?` -->
<!-- > multiple conditions could be supported using (?is.numeric) & (?"^a") for instance. -->
<!-- > Also the answer to a question is logical, not character -->
<!-- > means we cannot do `c("foo", ?is.numeric)` any more but we can do `s("foo", ?is.numeric)`  -->
<!-- > OR `names(.x) == "foo" | ?is.numeric` so not a big deal  -->
<!-- > var1:var2 should return a logical too for consistency, by overriding `:` (check if both symbols exist), or be available only in s() -->
...
Our solution is the same for cases.
...
> Here use by = TRUE, or maybe better by NA ? Or maybe change code so by doesn't have a default value, then use .by = NULL, or any value of length(0) to summarize without grouping, this is consistent as the more you add columns the more add rows.


```{r}
var <- "mass"
summarise(starwars, avg = mean(.data[[var]], na.rm = TRUE))
starwars[avg = mean(.subset[[var]], na.rm = TRUE), by=NA]
