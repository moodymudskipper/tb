question_mark <- function(expr){
  ...
  if(expression_contains_dots){
    ...
    nms_lgl <- eval(bquote(sapply(.data,  .(expr_fun))), envir = env$mask, enclos = parent.env(env))
  ...
  if(is.function(expr)){
    ...
    if(!is.logical(nms_lgl)) stop("The function used after `?` should return a logical output")
    names(.data)[nms_lgl]
  } else if (is.character(expr)) {
    if(length(expr) > 1) stop("`?` can't be followed by a character vector of length > 1")
    ...
    if(!is.logical(nms_lgl)) stop("The formula used after `?` should return a logical output")
    names(.data)[nms_lgl]
  }
  ## return all names that satisfy the expression
  # eval(bquote(names(.data)[sapply(.data, .(expr))]), env)
}
...
colon <- function(e1, e2){
  ...
  ## Fail if not found, this shouldn't happen
  if(!found) stop("The `[.tb` call wasn't found in the call stack")
  ## create env as `[.tb`'s environment, containing .data, and caller_env its parent
  env <- sys.frame(i)
